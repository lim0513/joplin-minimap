...
20. **A smooth scroll is fire-and-forget - always verify the landing.** `scrollIntoView({ behavior: 'smooth' })` animates towards an offset computed when it STARTS, and anything can cut it short or invalidate it: the reader nudging the wheel, Joplin re-rendering the note underneath, split view writing scrollTop of its own, images above the target finishing layout. The jump then stops part-way, which is exactly the bug report "clicked the H1 but it only scrolled up to 4.5". It is intermittent because it depends on what else happens during the animation.
    `jumpTo()` therefore starts the smooth scroll for the feel, then `watchJump()` polls the heading until its position stops changing and re-issues the scroll INSTANTLY with the same alignment. Idempotent when the smooth scroll already landed. The watcher bails out if the reader scrolls the note themselves - but NOT if they wheel the ToC list, which has its own handler and is a normal thing to do right after clicking a row.
    Do not "fix" this by just making the jump instant: the animation is what makes a long jump legible.
21. **Headless Chromium does not run smooth scrolling at all** - measured: `prefers-reduced-motion` is false, an instant `scrollIntoView` moves the scroller, and the smooth one leaves `scrollTop` untouched even 1.2s later. So the harness in this repo CANNOT reproduce or verify anything about the animation itself. It is still useful for this bug, as the worst case: if the correction lands the heading with the animation doing nothing whatsoever, it will also cover a scroll that merely stopped short.
22. **Windows + mounted-folder tooling:** writing these files through certain file-sync layers has truncated them mid-write before. After bulk edits, sanity-check with `node --check src/*.js build.js scripts/pack-jpl.js`.
